...
;*                                                                   *
;*   DDL manager code (non-overlaid & transient stuff)               *
;*                                                                   *
;*********************************************************************
...
EXTRN   ibp_offset:WORD,ibp_segment:WORD,ixb_offset:WORD,ixb_segment:WORD
EXTRN   plank_offset:WORD,plank_segment:WORD
...
; keep segment:offset of __ibpushtran, __ixblock, and __plankton
    push    cs
    pop es
    mov cs:search_flag,1    ; set flag to indicate one symbol search for exist
    mov di,OFFSET __ibp_text    ; es:di -> __IBPUSHTRAN symbol name
    call    ap_altent       ; check if exists
    or  al,al               ; see if found
    je  chk__ixblock        ; no, check for __ixblock

; found __ibpushtran
    call    get_c5_seg_off
    mov cs:ibp_offset,di
    mov cs:ibp_segment,ax

chk__ixblock:
    mov di,OFFSET __ixb_text    ; es:di -> __IXBLOCK symbol name
    call    ap_altent       ; check if exists
    or  al,al               ; see if found
    je  chk__plank          ; no
...
; found __ixblock
    call    get_c5_seg_off
    mov cs:ixb_offset,di
    mov cs:ixb_segment,ax

chk__plank:
    mov di,OFFSET __plank_text  ; es:di -> __PLANKTON symbol name
    call    ap_altent       ; check if exists
    or  al,al               ; see if found
    je  c5done              ; no
...
; found __plankton
    call    get_c5_seg_off
    mov cs:plank_offset,di
    mov cs:plank_segment,ax
...
__ibp_text  DB  14,'__IBPUSHTRAN',0
__ixb_text  DB  11,'__IXBLOCK',0
__plank_text    DB  12,'__PLANKTON',0
...
; return segment:offset of clipper 5.0x internal in ax:di
; upon entry, bx -> public entry of internal
; destroys ax,bx,di,ds

get_c5_seg_off  PROC    NEAR
    mov ds,bx               ; ds -> public entry
    xor bx,bx
...
gos_entloop:
	...
	mov ax,ds:[6]           ; get new overlay size
	add ax,15
	...
	shr ax,1                ; convert to paras
	shr ax,1                ; convert to paras
	cmp ax,cs:largest_ovl   ; check if should update largest overlay variable
	jb  chk_2nd             ; no

	mov bx,cs:largest_ovl   ; keep old largest value
	mov cs:largest_ovl,ax   ; update largest
	mov cs:second_ovl,bx    ; old largest to second largest
	jmp SHORT gos_next

chk_2nd:
	cmp ax,cs:second_ovl    ; check if should update second largest overlay
	jb  gos_next            ; no
	mov cs:second_ovl,ax
